...
class Move:
    def __init__(
        self, board, piece_id, piece_update: tuple[int, int, int, int], special=None
    ):
    ...
    def mega_move_func(self):
        ...
        if self.special is not None and self.special[0] == "=":
            promo_id = (
                self.special[1] if self.piece.color == "w" else self.special[1].lower()
            )
        ...
        if self.special is not None:
            if self.special == "e.p.":
                ...
                board_grid[captured_row][new_col] = Piece.create_piece(
                    "_", captured_row, new_col, self.board
                )
                move_id = f"{Board.coords(0, old_col)[1]}x{Board.coords(new_row, new_col)} e.p."

            elif self.special[0] == "O":
                # queenside else kingside
                old_rook_col, new_rook_col = (
                    (0, 3) if self.special == "O-O-O" else (7, 5)
                )
                board_grid[new_row][old_rook_col] = Piece.create_piece(
                    "_", new_row, old_rook_col, self.board
                )
                new_rook_id = "R" if self.piece.color == "w" else "r"
                board_grid[new_row][new_rook_col] = Piece.create_piece(
                    new_rook_id, new_row, new_rook_col, self.board
                )
        ...
        match self.piece.id.lower():
            case "p":
                move_id = (
                    new_coords if capture == "" else f"{old_coords[1]}x{new_coords}"
                )
            case "k":
                ...
                for piece_ids in search_moves:
                    ...
                    for row_move, col_move in search_moves[piece_ids]:
                        while True:
                            ...
                            if cur_square.id == self.piece.id and (
                                cur_row,
                                cur_col,
                            ) != (old_row, old_col):
...
class King(Piece):
    ...
    def generate_moves(self):
        ...
        new_board.grid[self.row][self.col] = Piece.create_piece(
            "_", self.row, self.col, new_board
        )
        ...
        for row_move, col_move in king_moves:
            ...
            if not (0 <= new_row < 8 and 0 <= new_col < 8):
                continue
            move_list.append(
                Move(new_board, self.id, (self.row, self.col, new_row, new_col))
            )
                ...
                if properties["can_castle"]:
                    ...
                    move_list.append(
                        Move(
                            new_board,
                            self.id,
                            (self.row, self.col, self.row, move_col),
                            side,
                        )
                    )
        return move_list
    ...
    def generate_moves(self):
        ...
        for row_move, col_move in queen_moves:
            ...
            while True:
                ...
                move_list.append(
                    Move(new_board, self.id, (self.row, self.col, cur_row, cur_col))
                )
...
class Knight(Piece):
    def generate_moves(self):
        ...
        for row_move, col_move in knight_moves:
            ...
            move_list.append(
                Move(new_board, self.id, (self.row, self.col, new_col, new_col))
            )
...
class Pawn(Piece):
    def generate_moves(self):
        ...
        board_grid[self.row][self.col] = Piece.create_piece(
            "_", self.row, self.col, new_board
        )
                ...
                for promo_code in "qrbn" if self.color == "b" else "QRBN":
                    move_list.append(
                        Move(
                            new_board,
                            self.id,
                            (self.row, self.col, push_row, self.col),
                            "=" + promo_code,
                        )
                    )
            else:
                move_list.append(
                    Move(new_board, self.id, (self.row, self.col, push_row, self.col))
                )
                dpush_row = self.row + 2 * row_move_direction
                if (
                    self.row == starting_row
                    and board_grid[dpush_row][self.col].id == "_"
                ):
                    move_list.append(
                        Move(
                            new_board,
                            self.id,
                            (self.row, self.col, dpush_row, self.col),
                        )
                    )
        for col_move in (1, -1):
            new_col = self.col + col_move
            if Piece.is_inbounds(push_row, new_col):
                if board_grid[push_row][new_col].id in enemies:
                    move_list.append(
                        Move(
                            new_board, self.id, (self.row, self.col, push_row, new_col)
                        )
                    )
